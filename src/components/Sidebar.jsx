...
export default function Sidebar({ etatSidebar, fermerSidebar, donneesAPI, chargement, erreur }) {
  ...
  // États locaux pour gérer la réponse et le chargement du LLM
  ...
  // 3. EFFET POUR LE DÉCLENCHEMENT CIBLÉ DU LLM
  useEffect(() => {
    if (!donneesAPI || !donneesAPI.main || !risk) return;

    let isMounted = true;

    // Appel LLM uniquement si le score de l'indice de sévérité dépasse 60
    if (risk.score > 60) {
      setLoadingAi(true);
      setAiAdvice('');
  ...
  return (
    <div 
      ...
      tabIndex="-1" 
      style={{ visibility: 'visible', display: 'block', zIndex: 1055 }} 
    >
      <div className="offcanvas-header border-bottom">
        <h5 className="offcanvas-title fw-bold">Météo Régionale</h5>
        <button 
          type="button" 
          className="btn-close" 
          aria-label="Close" 
      ...
      <div className="offcanvas-body">
        ...
        {donneesAPI && !chargement && !erreur && (
          <div>
            <h3 className="fw-bold text-dark">{donneesAPI.name}</h3>
            ...
            <ul className="list-unstyled bg-light p-3 rounded mb-4">
              <li className="mb-2"><strong>Humidité :</strong> {donneesAPI.main?.humidity}%</li>
              <li><strong>Vent :</strong> {donneesAPI.wind?.speed} m/s</li>
            </ul>

            {/* --- AFFICHAGE DE L'INDICE DE SÉVÉRITÉ --- */}
            {risk && (
              <div className="my-4 p-3 bg-light rounded border">
                <div className="d-flex justify-content-between small mb-2">
                  <span className="fw-semibold text-secondary">Indice de Sévérité</span>
                  <span style={{ fontWeight: 'bold', color: bleuInfo }}>{risk.score} / 100</span>
                </div>
                <div className="progress rounded-pill" style={{ height: '12px', backgroundColor: '#e9ecef' }}>
                  <div 
                    className="progress-bar rounded-pill" 
                    role="progressbar" 
                    style={{ width: `${risk.score}%`, backgroundColor: bleuInfo, transition: 'width 0.6s ease' }} 
                ...
                {/* INDICE DE SEVERITE */}
                <div className="mt-3">
                  <span className="badge" style={{ backgroundColor: `${bleuInfo}18`, color: '#0993af', border: `1px solid ${bleuInfo}`, padding: '8px 14px', fontSize: '0.85rem', fontWeight: '600' }}>
                    {risk.label}
                  </span>
                </div>
              </div>
            ...
            <div className="my-4 p-2 bg-white rounded border">
              <h6 className="fw-bold mb-3 small text-uppercase text-muted">Évolution - 7 derniers jours</h6>
              <div style={{ width: '100%', height: 180 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={donneesGraphique} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <defs>
                      <linearGradient id="colorTemp" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor={bleuInfo} stopOpacity={0.4}/>
                        <stop offset="95%" stopColor={bleuInfo} stopOpacity={0.0}/>
                      </linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f0f0f0" />
                    <XAxis dataKey="jour" tick={{ fontSize: 11 }} stroke="#888888" />
                    <YAxis 
                      tick={{ fontSize: 11 }} 
                      stroke="#888888" 
            ...
            {/* --- IA  --- */}
            <div className="p-3 rounded shadow-sm" style={{ backgroundColor: '#f8f9fa', borderLeft: `4px solid ${bleuInfo}` }}>
              <strong className="d-block mb-2 text-info small text-uppercase tracking-wider">Analyse de l'IA :</strong>
              {loadingAi ? (
                ...
                <p className="mb-0 small text-secondary" style={{ lineHeight: '1.6' }}>
                  {aiAdvice}
                </p>
              )}
            </div>
